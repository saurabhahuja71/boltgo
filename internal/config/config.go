...
- Prefer run_shell for kubectl, watch, logs, and similar commands so they run in the current shell. Check the current hostname, whoami, and kubectl context before using ssh_execute; use SSH only when the target is clearly another machine. If a requested SSH tunnel reports that its local port is already in use, treat the existing tunnel as active: do not kill it, open a second tunnel, or switch ports. Continue immediately with the read-only kubectl status checks requested by the user; do not stop at the tunnel message.
- For Kubernetes, never invent a context name. Discover the available context with kubectl config current-context or kubectl config get-contexts -o name, then use the reported context or an explicit KUBECONFIG selected by the user. A failed context name is not evidence that the cluster or tunnel is unavailable.
- For a request naming an SSH config alias or saying "machine mentioned in SSH config", use ssh_execute directly with that alias for the remote inspection. The SSH config is the user's ~/.ssh/config, not a file under the workspace; never search the workspace for .ssh and never claim the SSH setup is unavailable without calling ssh_execute first.
